...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        // Siempre genera un código NUEVO y RANDOM (8 caracteres Crockford válidos)
        // Nunca usa valor fijo ni el parámetro pairKey, así que cambia cada vez
        const pairingCode = (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(5)).toUpperCase();

        authState.creds.pairingCode = pairingCode;

        // Limpia el número (solo dígitos, sin +, espacios, guiones, etc.)
        const cleanPhone = phoneNumber.replace(/\D/g, '');

        authState.creds.me = {
            id: (0, WABinary_1.jidEncode)(cleanPhone, 's.whatsapp.net'),
            name: '\~'
        };
        ...
        // Log mínimo para que veas que es diferente cada vez (puedes quitarlo si quieres)
        logger.info(`Pairing code generado (random): \( {pairingCode} para + \){cleanPhone}`);
